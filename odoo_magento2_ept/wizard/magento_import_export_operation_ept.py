...
class MagentoImportExportEpt(models.TransientModel):
    ...
    # from odoo product to magento layer ganesh code
    ...
    # from odoo to magento layer ganesh code
    def prepare_magento_template_values_ept(self, product_dict, template):
        ...
        m_values = {
            ...
            'export_product_to_all_website': False,
            'product_expiry': template.product_expiry,
            'magento_product_barcode': template.barcode,
            'brand': template.magento_brand.id,
            # 'brand': [brand_id.id for brand_id in template.magento_brand],
        }

        magento_tax = False
        if template.taxes_id:
            for tax_id in template.taxes_id:
                if tax_id.name == 'BTW(9.0%)':
                    magento_tax_id = self.env['magento.tax.class'].search(
                        [('magento_tax_class_name', '=', 'Laag tarief')], limit=1)
                    magento_tax = magento_tax_id.id
                elif tax_id.name == 'BTW(21.0%)':
                    magento_tax_id = self.env['magento.tax.class'].search(
                        [('magento_tax_class_name', '=', 'Hoog tarief')], limit=1)
                    magento_tax = magento_tax_id.id
                else:
                    magento_tax = False
                break
        m_values.update({
            'magento_tax_class': magento_tax
        })
