...
class MagentoExportProductEpt(models.TransientModel):
    ...
    # from magento odoo to magento backend
    def prepare_product_data(self, product, is_child, product_type, attribute_set, website_ids):
        ...
        stocks = self.env['stock.quant'].search([('product_id', '=', product.odoo_product_template_id.id)])
        if stocks:
            qty = [stock.quantity for stock in stocks[0]]
            qty = int(qty[0])
        else:
            qty = 0
        ...
        ean = product.magento_product_barcode
        if product.brand:
            merk_brand = str(product.brand.magento_attribute_option_id)
        else:
            merk_brand = False
        ...
        data = {
            "product": {
                ...
                "custom_attributes": [
                    {'attribute_code': 'ean', 'value': str(ean)},
                    {'attribute_code': 'expiry_date', 'value': str(expiry)},
                    {'attribute_code': 'cost_price', 'value': str(cost)},
                ]
            }
        }

        if merk_brand:
            merk_class = {'attribute_code': 'merk', 'value': merk_brand}
            data.get('product', dict()).get('custom_attributes').append(merk_class)
