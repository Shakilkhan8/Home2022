...
class SaleShop(models.Model):
    ...
    def _woocom_kanban_dashboard(self):
        ...
        all_invoice_ids = invoice_obj.search([('is_woocom','=',True),('invoice_origin', 'in', origin_list)])
        pending_invoice_ids = invoice_obj.search([('is_woocom','=',True),('invoice_origin', 'in', origin_list),('state','!=','paid')])
        complete_invoice_ids = invoice_obj.search([('is_woocom','=',True),('invoice_origin', 'in', origin_list),('state','=','paid')])
        draft_invoice_ids = invoice_obj.search([('is_woocom','=',True),('invoice_origin', 'in', origin_list),('state','=','draft')])
        cancel_invoice_ids = invoice_obj.search([('is_woocom','=',True),('invoice_origin', 'in', origin_list),('state','=','cancel')])
...
#         'late_delivey':late_delivey_ids,
    ...
    def action_view_all_invoice(self):
        ...
        all_order_ids = order_obj.search([('shop_id', '=', self[0].id)])
        origin_list = [s.name for s in all_order_ids]
        invoice_id = invoice_obj.search([('is_woocom', '=', True), ('invoice_origin', 'in', origin_list)])
        imd = self.env['ir.model.data']
    ...
    def action_view_pending_invoice(self):
        ...
        all_order_ids = order_obj.search([('shop_id', '=', self[0].id)])
        origin_list = [s.name for s in all_order_ids]
        invoice_id = invoice_obj.search([('is_woocom', '=', True),('state','=','open'),('invoice_origin', 'in', origin_list)])
        imd = self.env['ir.model.data']
        action = imd.xmlid_to_object('account.action_move_out_invoice_type')
        print("<><><><><><><><>ACTION<><><><><><><><>", action)
        list_view_id = imd.xmlid_to_res_id('account.invoice_tree')
    ...
    def action_view_cancel_invoice(self):
        order_obj = self.env['sale.order']
        invoice_obj = self.env['account.move.send']
        all_order_ids = order_obj.search([('shop_id', '=', self[0].id)])
        origin_list = [s.name for s in all_order_ids]
        invoice_id = invoice_obj.search(
